...
export async function getInspectionReportData(): Promise<InspectionReportData> {
  ...
  const [reportsResult, itemsResult, measurementsResult, detailsResult, codesResult] =
    await Promise.all([
      ...
      supabase.from("code_details").select("seq, code, code_name, code_groups!inner(group_code)").eq("is_active", true).in("code_groups.group_code", ["PRODUCT_TYPE", "FINAL_JUDGMENT_STATUS"]).order("sort_order").order("seq"),
    ]);
