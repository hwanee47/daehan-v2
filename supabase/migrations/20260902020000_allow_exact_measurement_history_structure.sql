...
declare
  ...
  supplied_existing_distinct_count integer;
  affected_count integer;
  next_run_no integer;
  normalized_rows jsonb := '[]'::jsonb;
  row_value jsonb;
  row_sort_order integer := 0;
  run_seq bigint;
  nominal_text text;
...
begin
  ...
  select count(*), count(distinct (entry.value ->> 'item_seq')::bigint)
  into supplied_existing_count, supplied_existing_distinct_count
  from jsonb_array_elements(p_rows) as entry(value)
  where nullif(entry.value ->> 'item_seq', '') is not null;

  if supplied_existing_count <> supplied_existing_distinct_count then
    raise exception 'duplicate measurement item' using errcode = '22023';
  end if;
  if exists (
  ...
  for row_value in select value from jsonb_array_elements(p_rows) loop
    row_sort_order := row_sort_order + 1;
    ...
    if char_length(coalesce(nominal_text, '')) > 100
      ...
      raise exception 'inspection item field too long' using errcode = '22023';
    end if;
    ...
    if tolerance_min_is_number and tolerance_max_is_number and tolerance_min_number > tolerance_max_number then
      raise exception 'invalid inspection item tolerance order' using errcode = '22023';
    end if;

    row_value := jsonb_set(row_value, '{sort_order}', to_jsonb(row_sort_order), true);
  ...
  where x.item_seq is not null
    and m.inspection_report_seq = p_inspection_report_seq
    and m.inspection_report_item_seq = x.item_seq;
  get diagnostics affected_count = row_count;
  if affected_count <> supplied_existing_count then raise exception 'measurement update count mismatch' using errcode = '22023'; end if;
  ...
  select run_seq, x.item_seq, x.sort_order,
    nullif(btrim(x.nominal_dimension), ''), nullif(btrim(x.tolerance_min), ''), nullif(btrim(x.tolerance_max), ''),
    i.marker_x_ratio, i.marker_y_ratio,
    x.result_1, x.result_2, x.result_3, x.result_4, x.result_5, x.result_6,
    x.result_7, x.result_8, x.result_9, x.result_10, nullif(btrim(x.note), ''), actor_id, actor_id
  from jsonb_to_recordset(normalized_rows) as x(
    sort_order integer, item_seq bigint, nominal_dimension text, tolerance_min text, tolerance_max text,
    result_1 numeric, result_2 numeric, result_3 numeric, result_4 numeric, result_5 numeric,
    result_6 numeric, result_7 numeric, result_8 numeric, result_9 numeric, result_10 numeric, note text
  )
  left join public.inspection_report_items i
    on i.seq = x.item_seq and i.inspection_report_seq = p_inspection_report_seq
  order by x.sort_order;

  return jsonb_build_object(
    'run_seq', run_seq,
    'item_seqs', (
      select coalesce(jsonb_agg(entry.value -> 'item_seq'), '[]'::jsonb)
...
comment on function public.save_inspection_measurement_entry(bigint, bigint, text, text, text, text, text, text, text, text, text, text, text, text, jsonb) is
  '현재 원본 항목의 일부 또는 이력 전용 구조를 허용하며 입력행 전체의 측정 이력 스냅샷을 저장한다';
