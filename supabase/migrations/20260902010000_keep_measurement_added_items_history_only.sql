...
begin
  ...
  if supplied_existing_count <> current_item_count
    or supplied_existing_distinct_count <> current_item_count then
  ...
  if affected_count <> current_item_count then raise exception 'measurement update count mismatch' using errcode = '22023'; end if;
...
comment on function public.save_inspection_measurement_entry(bigint, bigint, text, text, text, text, text, text, text, text, text, text, text, text, jsonb) is
  '검사성적서 원본은 변경하지 않고 기존 항목 최신값과 입력행 전체의 측정 이력 스냅샷을 저장한다';
